...
namespace ark_vrf_cpp {
    extern "C" {
        ...
        int ring_vrf_verify(size_t ring_size, const void *comm_ptr, size_t comm_len, const void* sig_ptr, size_t sig_len,
    ...
    {
        return ring_vrf_output(out.data(), out.size(), sig.data(), sig.size());
    }

    inline int ietf_vrf_output(const std::span<uint8_t, 32> &out, const std::span<const uint8_t> &sig)
    {
        return ietf_vrf_output(out.data(), out.size(), sig.data(), sig.size());
    }
}
