...
namespace turbo::cli::jamnp_client {
    struct cmd: command {
        void configure(config &cmd) const override
        {
            cmd.name = "jamnp";
            cmd.desc = "Execute a JAMNP client request";
            ...
            cmd.args.expect({"<request-name>"});
        }

        void run(const arguments &args, const options &opts) const override
        {
            const auto &request = args.at(0);
            ...
            auto cert = jamnp::make_cert(key_pair);
            jamnp::address_t server_addr{*opts.at("host"), from_str<uint16_t>(opts.at("port").value().c_str())};
            logger::info("connecting to {}", server_addr);
            jamnp::client_t<config_tiny> client{server_addr, "turbojam", "jamnp-s/0/b5af8eda", std::move(cert)};
            logger::info("created a JAMNP client for {} request", request);
            if (request == "fetch-blocks") {
                logger::info("fetch-blocks: {}", client.fetch_blocks({}, 10).wait());
            } else {
                throw error(fmt::format("unsupported request: {}", request));
            }
        }
    };
